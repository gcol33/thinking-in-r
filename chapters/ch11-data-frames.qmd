...
Type theory calls this a *product type*. A data frame with columns `name` (character) and `mass` (double) has type `character × double`: each row pairs one character value with one double value. Add a third column and the type grows: `character × double × integer`. So what goes into those columns when you build one from scratch?
