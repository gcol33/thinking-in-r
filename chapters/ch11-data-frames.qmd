...
That structure is the data frame\index{data frame}: a list where every element is a vector of the same length. Lists hold anything; a data frame constrains that freedom to equal-length columns, and once you see this, subsetting, column operations, and the entire tidyverse make sense. The idea predates R by decades: Edgar Codd's\index{Codd, Edgar} 1970 paper at IBM defined relations as sets of tuples, each row a typed record, and R's data frames, SQL tables, pandas DataFrames, and Spark datasets all descend from that same relational algebra\index{relational model}.
...
Type `"list"`, because it *is* a list. The difference from an ordinary list is that every element (every column\index{column}) must be a vector of the same length, so each row\index{row} lines up across all columns; one observation, one position in every vector. Build one yourself:
