...
A domain-specific language\index{domain-specific language (DSL)}, or DSL, is a small language designed for one task. SQL queries data, regular expressions match text, and HTML structures documents. These are *external* DSLs with their own parsers, their own syntax, their own tooling.

R excels at something different: *internal* DSLs, mini-languages embedded inside R itself, using R's own syntax but giving it new meaning. The formula interface, ggplot2's grammar of graphics, dplyr's verb chains, data.table's `[i, j, by]` notation: each lives inside R and plays by R's rules (mostly).

By the mid-1960s, programmers were drowning in general-purpose syntax that forced every domain into the same mold. Peter Landin argued in 1966 that they should be able to build "a language appropriate to the problem." R's formula interface, which dates to S in the 1980s, shares the same instinct: capture code, give it new meaning.
...
The technique varies, but the principle does not. Capture code, give it new meaning. What happens when you try this yourself?
...
The `+.gg` method, meanwhile, simply takes the existing plot object, attaches the new component (a layer, a scale, a theme), and returns the modified plot. This is an expression builder\index{expression builder} pattern: the plot is a list that grows with each `+`, and rendering happens only when you print it.
...
The formula interface from the opening of this chapter, where `+` stopped meaning addition and started meaning "include this predictor" — you now know exactly how that trick works: an S3 class, a method for `+`, and a function that interprets the captured expression in a new context.
