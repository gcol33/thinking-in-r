...
For functional programming, `purrr::safely()` and `purrr::possibly()` wrap a function to make it error-tolerant. Together with `tryCatch()`, these form R's condition\index{condition} handling system. `safely()` returns a list with `$result` and `$error`, while `possibly()` returns a default value on failure. Both are designed for use with `map()` when applying a function that might fail on some elements:
