...
Notice that `sqrt` has no parentheses. You are not calling `sqrt` here; you are handing the function itself to `sapply`, which calls it five times, once per element. This is the same distinction from @sec-programs-and-proofs: `fact` is the definition (a rule), while `fact(n)` is the rule applied to an argument.
...
A function that takes another function as an argument\index{function!as argument} is called a *higher-order function*\index{higher-order function}. `sapply` is one; most of R's power comes from others.
...
`make_adder` is a *function factory*: it takes a number `n` and returns a *new function*\index{function!as return value} that adds `n` to its argument:
