...
Always 1. The counter never advances, because every call starts from scratch, every local variable is born and dies within the span of a single invocation. You could call it a thousand times and nothing would accumulate. So how do you build a function that *remembers*? A function that counts, that learns, that carries the residue of its past? The answer runs through environments, scoping rules, and a mechanism with a name borrowed from mathematical logic: the closure.
...
The result is 10, not 20. `f` was defined in the global environment, so it looks for `x` there, completely ignoring the `x <- 20` inside `g` where it was merely *called*. This is lexical scoping: a function resolves names where it was *defined*, not where it was *called*, and R inherited the idea from Scheme (1975), which adopted it over Lisp's dynamic scoping and changed the trajectory of programming language design. Dynamic scoping searches the call stack, which is simpler to implement but makes functions unpredictable, because the same function returns different results depending on who calls it. R took lexical scoping from Scheme, not from S (which originally used dynamic scoping; see @sec-s-language), and this one design decision is why closures work in R at all.

The principle is simple. The interactions are not.
