...
That difference has a name: metaprogramming\index{metaprogramming}. Code that operates on code. You have been using it from your first formula, your first call to `aes()`, your first dplyr pipeline, all of it built on the machinery this chapter describes.
...
In @sec-church, Church's lambda calculus treated functions as data that could be passed around and applied. R extends that principle to *all code*, not only functions. An expression is a value, exactly like a number or a string. In @sec-operators-are-functions, you saw that `+` is a function and `2 + 3` is a function call. That function call is also a data structure you can capture and manipulate before it ever executes.

R inherited this idea from Lisp, where `(quote (+ 1 2))` returns the list `(+ 1 2)` instead of computing 3. A language whose code has the same structure as its data is called *homoiconic*: Lisp code is lists, Lisp data is lists; R code is call objects (trees), and R data includes call objects.

R's homoiconicity means it gets this for free. Python, Java, and C++ are not homoiconic: their code is text, so metaprogramming means parsing strings, which is fragile. Python's `inspect.getsource()` returns a string you then have to parse; R's `quote(x + 1)` gives you the syntax tree directly — a call object you can subset with `[[`, modify with replacement, and evaluate in any environment you choose. That tree is an ordinary R object. It lives in memory, responds to R's standard operations, and you can pass it to functions like any vector or list. Tidy evaluation, formula interfaces, and ggplot2's `aes()` all rest on that foundation.
...
The rlang equivalents are `enexpr()` (captures an expression) and `enquo()`\index{enquo()} (captures an expression plus its environment, forming a *quosure*). The decision rule is simple. If your function will be called by dplyr or other tidy-evaluation-aware code, use `enexpr()`/`enquo()`, because they integrate with `!!`, `{{ }}`, and `eval_tidy()`. For standalone base R, `substitute()` and `eval()` are sufficient and carry zero dependencies.
...
Wickham borrowed the concept directly from formulas, and their 30-year track record in R's modeling ecosystem is what made the design credible.
:::
