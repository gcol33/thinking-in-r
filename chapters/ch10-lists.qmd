...
`species` is a character scalar, `mass` is a numeric scalar, `measurements` is a numeric vector of length 3.
...
Each `[[` or `$` steps one level deeper: `study$species` is a list of three lists, `study$species[[1]]` is the first of those, and `study$species[[1]]$name` is the string `"Adelie"`. JSON from a web API, the output of `lm()`, and configuration files are all nested lists or objects built on them.
...
A linked list is a chain of nodes, each holding a value and a pointer to the next. Prepending is O(1); reaching element *k* means following *k* pointers. You build one in R with nested lists:
