...
A dplyr\index{dplyr (package)} verb takes a data frame and returns a data frame: it goes in first, comes out the same shape, and nothing else changes. Five verbs cover most of what you need:
...
You compose them for complex transformations (via pipes, @sec-pipes-and-composition), but each one does exactly one thing.
...
What makes dplyr's version distinctive is that the partition is invisible metadata attached to the data frame — `group_by()` changes nothing you can see in a printout, but it changes everything `summarise()` does. That silent metadata is powerful, and it is also the source of the most common grouping bugs.
