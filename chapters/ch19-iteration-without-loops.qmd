...
Functions are values (@sec-functions-are-values-chapter) and you can pass them to other functions (@sec-passing-functions).
...
A *functional* builds on that idea: it is a higher-order function that handles the iteration\index{iteration} for you. You give it a function and a collection; it applies the function to each element, manages the indexing and pre-allocation, and returns the results.
...
Three reasons. First, typed variants: `map_dbl()`\index{map\_dbl()} returns a double vector, `map_chr()`\index{map\_chr()} returns a character vector, and if the function returns the wrong type, you get an error. No more guessing, no more silent simplification.
...
Third, you may encounter the formula shorthand `~ mean(.x$body_mass_g, na.rm = TRUE)`, purrr's anonymous function syntax from before R 4.1 introduced `\(x)`. You will still see it in existing codebases, but prefer `\(x)` in new code.

`map()` always returns a list and `map_dbl()` always returns a double vector, regardless of the data. This type stability is the core advantage over `sapply()`, and it matters more than it sounds: in a long pipeline, knowing the exact shape of every intermediate result is the difference between debugging for five seconds and debugging for an hour.
...
With `map()`, the list has three elements, so the function runs three times; the structure itself determines how many applications occur. You stop managing the iteration and let the data's shape manage it for you.
...
- `map_lgl(.x, .f)`\index{map\_lgl()}: each call must return a single logical. Result is a logical vector.
- `map_int(.x, .f)`\index{map\_int()}: each call must return a single integer. Result is an integer vector.
...
**When iterations depend on each other.** If the result of iteration `i` feeds into iteration `i + 1`, a functional cannot help you, because functionals assume independence between elements. Simulation chains, iterative algorithms, and accumulating state all need explicit loops. (purrr's `reduce()` and `accumulate()` handle some of these sequential cases, and you will meet them in @sec-reduce-and-accumulate.)
...
In @sec-column-wise, you saw `across()`\index{across()} for applying a function to multiple columns inside a dplyr verb. So what is `across()`, exactly? It is `map()` thinking applied inside a data frame pipeline.
