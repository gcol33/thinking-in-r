...
The fix is structural, not behavioral. Do not rely on discipline ("I will remember to run script 3 first"); build a system that makes the wrong thing hard. Each tool in this chapter eliminates one failure mode, and the first one costs nothing to adopt.
...
`renv::init()` creates a private library for this project, isolating your package versions from every other project on your machine. `renv::snapshot()` writes `renv.lock`\index{lockfile}, a JSON file listing every package and its exact version: your dependency manifest. `renv::restore()` reads that lockfile and installs exactly those versions, so a collaborator who clones your repo and runs `renv::restore()` gets the same packages you used.

The workflow: `init()` once, `snapshot()` after installing or updating packages, commit `renv.lock` to Git. That is all.

renv does not version R itself. If the R version matters, document it; Docker solves this completely (see @sec-reproducibility-stack), but is beyond this book. The `renv/` directory itself, which contains the actual installed packages, is typically git-ignored.
...
If you know R Markdown, you already know Quarto. The main differences: Quarto uses `#|` for chunk options instead of the chunk header, supports Python, Julia, and Observable in addition to R, and has better defaults for academic output (cross-references, citations, callouts).
...
Each target is a function call. Functions are the unit of computation (@sec-functions-are-values-chapter); targets are the unit of caching. If `raw_data` has not changed, `clean_data` does not re-run. If you change `plot_results()`, only `fig` re-runs. This matters most when a step is expensive: if a model takes an hour to fit, `targets` caches the result and re-fits only when the model code or its inputs change, so tweaking a color on a plot re-runs the plot in seconds off the cached model.
...
`tar_visnetwork()` produces a dependency graph showing which targets are up to date (green), outdated (blue), or errored (red), which makes debugging complex pipelines far easier than staring at a collection of numbered scripts.
