...
| Must run on a different OS | Docker |

Each layer solves a specific failure mode. Together, they make "works on my machine" a non-issue. Beyond Docker, the Nix package manager (via the `rix` R package) treats packages as pure functions from inputs to outputs: fix all inputs, get deterministic output. Where `renv` captures R package versions, Nix captures the entire system stack, including R itself, system libraries, and the compiler. The principle is the same as referential transparency applied to builds.
