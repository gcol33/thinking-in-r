...
A function factory\index{function factory} is a function that returns a function:
...
Here is a bug that will cost you an afternoon if you have never seen it before. R evaluates arguments lazily, meaning they are not computed until they are actually used, and this seemingly innocent optimization has consequences for factories that are anything but innocent (the *lazy evaluation trap*\index{lazy evaluation trap}):
...
Sometimes you already have the right function, and the only problem is speed. Memoization\index{memoization} wraps a function so it caches its results: the first call computes normally, but subsequent calls with the same arguments return the cached value instantly, skipping the computation entirely.
